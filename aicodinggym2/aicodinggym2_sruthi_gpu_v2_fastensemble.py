"""
- Sparse TF-IDF Ridge as a strong CPU baseline.
...
CV_SEEDS = [int(x) for x in os.environ.get("AES_CV_SEEDS", "42").split(",") if x.strip()]
SVD_COMPONENTS = int(os.environ.get("AES_SVD_COMPONENTS", "256"))
THRESH_MAXITER = int(os.environ.get("AES_THRESH_MAXITER", "1500"))
THRESH_RESTARTS = int(os.environ.get("AES_THRESH_RESTARTS", "2"))
ENSEMBLE_GRID_STEP = float(os.environ.get("AES_ENSEMBLE_GRID_STEP", "0.05"))
ENSEMBLE_TOP_K = int(os.environ.get("AES_ENSEMBLE_TOP_K", "25"))
...
def optimise_thresholds(y_true, raw, n=6, maxiter=None, restarts=None):
    """QWK threshold optimizer.

    The original v2 used 11 Nelder-Mead runs × 20,000 max iterations every
    time this function was called. That is far too slow inside ensemble search.
    Defaults here are intentionally lighter and can be overridden with env vars:
        AES_THRESH_MAXITER=3000 AES_THRESH_RESTARTS=4
    """
    raw = np.asarray(raw, dtype=np.float64)
    maxiter = THRESH_MAXITER if maxiter is None else maxiter
    restarts = THRESH_RESTARTS if restarts is None else restarts

    def neg_qwk(t):
        return -qwk(y_true, apply_thresholds(raw, np.sort(t), n=n))

    base_inits = [
        np.array([1.5, 2.5, 3.5, 4.5, 5.5], dtype=np.float64),
    ...
    return np.sort(best_x)


def threshold_candidates_from_raw(raw):
    raw = np.asarray(raw, dtype=np.float64)
    return [
        np.array([1.5, 2.5, 3.5, 4.5, 5.5], dtype=np.float64),
        np.percentile(raw, [8, 25, 55, 82, 94]),
        np.percentile(raw, [10, 30, 55, 80, 93]),
        np.linspace(raw.min(), raw.max(), 7)[1:-1],
    ]


def quick_score_threshold_sets(y_true, raw, threshold_sets):
    best_s = -1.0
    best_t = None
    for t in threshold_sets:
        s = qwk(y_true, apply_thresholds(raw, np.sort(t)))
        if s > best_s:
            best_s = s
            best_t = np.sort(t)
...
# 1) score every weight blend with a small fixed bank of threshold candidates;
# 2) run expensive Nelder-Mead threshold optimization only on the top K blends.
threshold_bank = []
for raw in [oof_ridge, oof_svr, oof_xgb_reg, oof_xgb_cls,
            0.25 * oof_ridge + 0.25 * oof_svr + 0.25 * oof_xgb_reg + 0.25 * oof_xgb_cls]:
    threshold_bank.extend(threshold_candidates_from_raw(raw))

top_candidates = []
grid = np.arange(0.0, 1.0001, ENSEMBLE_GRID_STEP)
checked = 0
for w_r in grid:
    for w_s in grid:
        for w_g in grid:
            w_c = round(1.0 - w_r - w_s - w_g, 6)
            if w_c < -1e-9 or w_c > 1 + 1e-9:
                continue
            if w_g + w_c < 0.25:
                continue
            ...
            s, t = quick_score_threshold_sets(y_train_int, blend, threshold_bank)
            top_candidates.append((s, (w_r, w_s, w_g, w_c), t))
            checked += 1

# Keep only the most promising blends before running slow threshold optimization.
top_candidates.sort(key=lambda x: x[0], reverse=True)
top_candidates = top_candidates[:ENSEMBLE_TOP_K]
print(f"  checked {checked} weight blends quickly; optimizing thresholds for top {len(top_candidates)}")

for rank, (_, weights, _) in enumerate(top_candidates, start=1):
    w_r, w_s, w_g, w_c = weights
    blend = w_r * oof_ridge + w_s * oof_svr + w_g * oof_xgb_reg + w_c * oof_xgb_cls
    ...
    if s > best_score:
        ...
        best_thresh = t
        print(
            f"  new best #{rank}: QWK={best_score:.5f} "
            f"Ridge={w_r:.2f} SVR={w_s:.2f} XGB-reg={w_g:.2f} XGB-cls={w_c:.2f}"
        )
